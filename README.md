...
- 插件内置7424个Ozon官方简体中文类目及采集必需规则；中文名称来自Seller API `ZH_HANS`并按`category_id/type_id`与俄文树精确配对，禁止本地翻译或猜测。插件优先读取主电脑最新官方缓存、断网时回退到同版本官方内置缓存；缺失字典值标记为`unknown`而不是阻断采集。

同步官方中文类目（只读，不创建商品、不查询库存）：

```bash
.venv/bin/python scripts/build_category_zh_cache.py --shop zhonglian1
```
