  ...
  if (message?.type === 'FACTORY_FETCH_DETAIL_PAGE') {
    (async () => {
      try {
        ...
        if (!response.ok) throw new Error(`详情页读取失败 HTTP ${response.status}`);
  ...
  if (message?.type === 'FACTORY_FETCH_1688_PAGE_SOURCE') {
    ...
    (async () => {
      try {
        ...
        if (!response.ok) throw new Error(`1688商品页读取失败 HTTP ${response.status}`);
        const body = await response.text();
        if (!body || body.length > 2_000_000) throw new Error("1688商品页源码为空或超出安全大小");
