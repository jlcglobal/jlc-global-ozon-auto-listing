...
function seerfarText(node: Element | null | undefined) {
  return String(node?.textContent || "").replace(/\s+/g, " ").trim();
}

function seerfarNumber(value: unknown) {
  const text = String(value || "").replace(/[^0-9,.-]/g, "").replace(/,/g, ".");
  const number = Number(text);
  return Number.isFinite(number) ? number : 0;
}

function isUsableMarketKeyword(value: unknown) {
...
async function factoryRequest(path: string, options: { method?: string; headers?: Record<string, string>; body?: string } = {}) {
  return new Promise<any>((resolve, reject) => {
    chrome.runtime.sendMessage({ type: "FACTORY_FETCH", path, options }, (result) => {
      const runtimeError = chrome.runtime.lastError;
      if (runtimeError) return reject(new Error(runtimeError.message));
      if (!result?.ok) return reject(new Error(result?.error || "工作台连接失败"));
      let body: any = {};
...
function assignNativeValue(input: HTMLInputElement | HTMLTextAreaElement, value: string) {
...
function findSearchInput(mode: string) {
  if (mode === "keyword_miner") {
    return document.querySelector<HTMLInputElement>("#magnet-keyword");
  }
  if (mode === "keyword_reverse") {
    const reverseInput = document.querySelector<HTMLInputElement>("#reverse-keyword + .select2 input.select2-search__field");
    if (reverseInput) return reverseInput;
  }
  const candidates = Array.from(document.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>("textarea, input[type='text'], input:not([type])"));
  if (mode === "keyword_reverse") {
...
function findSearchButton(mode: string, input: HTMLInputElement | HTMLTextAreaElement | null) {
  if (mode === "keyword_miner") {
    return document.querySelector<HTMLElement>("#tab-keyword-magnet button.quick-search");
  }
  const reverseSelect = document.querySelector<HTMLElement>("#reverse-keyword");
  return reverseSelect?.closest(".row")?.querySelector<HTMLElement>("button.quick-search")
    || input?.closest(".row")?.querySelector<HTMLElement>("button.quick-search")
    || null;
}

function setReverseSku(value: string) {
  const select = document.querySelector<HTMLSelectElement>("#reverse-keyword");
  if (!select) return false;
  const option = new Option(value, value, true, true);
  select.replaceChildren(option);
  select.dispatchEvent(new Event("change", { bubbles: true }));
  const jquery = (window as any).jQuery || (window as any).$;
  if (jquery) jquery(select).trigger("change");
...
function parseKeywordRows() {
  ...
  if (!table) return [];
  const indexOf = (label: string) => table.headers.findIndex((header) => header.includes(label));
  const queryIndex = indexOf("关键词");
  const relatedProductIndex = indexOf("关键词相关商品");
  const heatIndex = indexOf("月搜热度");
  if (queryIndex < 0 || heatIndex < 0) return [];
  ...
  return table.rows.map(({ cells, values }) => {
    ...
    for (const [key] of fields) {
      ...
      if (index >= 0 && values[index]) row[key] = seerfarNumber(values[index]);
    }
    if (relatedProductIndex >= 0 && cells[relatedProductIndex]) {
      row.related_product_urls = Array.from(cells[relatedProductIndex].querySelectorAll<HTMLAnchorElement>("a[href*='/product/']"))
        .map((link) => link.href)
...
function parseReverseRows() {
  ...
  for (const table of tables) {
    ...
    if (queryIndex < 0 || countIndex < 0) continue;
...
function keywordRowsSignature(mode: string) {
...
async function waitForInitialResultsToSettle(mode: string, timeoutMs = 6000) {
...
function waitForKeywordRows(
  ...
  return new Promise<Array<Record<string, unknown>>>((resolve) => {
    ...
    const timer = window.setInterval(() => {
      ...
      if (rows.length && resultChanged && signature === lastSignature) stableCount += 1;
      else stableCount = 0;
...
async function runSeerfarJob(job: Record<string, unknown>) {
  ...
  if (!input || !button) throw new Error(`没有找到 Seerfar ${mode === "keyword_reverse" ? "关键词反查" : "关键词挖掘"}的输入框或查询按钮`);
  ...
  if (mode === "keyword_reverse") {
    if (!setReverseSku(seedKeyword)) throw new Error("没有写入 Seerfar 反查 SKU");
  } else {
    assignNativeValue(input, seedKeyword);
    if (input.value !== seedKeyword) throw new Error("没有写入 Seerfar 挖掘关键词");
  }
  button.click();
