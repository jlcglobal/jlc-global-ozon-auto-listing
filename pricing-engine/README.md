...
It reads only the `RETS` worksheet from `pricing-engine/shipping_rules.xlsx`. RETS refers to the logistics provider 俄通收; this workbook is not a universal marketplace shipping tariff. Users must review and update it against their own current RETS contract and quotation. Other workbook sheets are ignored.
