...
$ShopRegistry = Join-Path $ProjectRoot "ozon-adapter\shops.json"
$ShopExample = Join-Path $ProjectRoot "ozon-adapter\shops.example.json"
if ((-not (Test-Path $ShopRegistry)) -and (Test-Path $ShopExample)) {
    Copy-Item $ShopExample $ShopRegistry
    Write-Host "已创建本地店铺配置模板；请在工作台设置中填写自己的 Ozon 密钥。" -ForegroundColor Yellow
}
